...
public interface InvoiceRepository extends JpaRepository<Invoice, UUID> {
	 ...
	 * Returns the number of rows affected (1 if successful, 0 if invoice not
	 * found).
	 *
	 * @param invoiceId The UUID of the invoice to update
	 * @param status    The new status to set (e.g., "DRAFT", "SUBMITTED",
	 *                  "AUTHORISED")
	...
	/**
	 * Counts the number of invoices with a given status for a specific
	 * organization.
	 * Used to populate the organization dashboard with invoice status breakdowns.
	 *
	 * @param status       The invoice status to filter by (e.g., "DRAFT",
	...
	/**
	 * Retrieves the next value from the invoice_number_seq database sequence.
